...
the composite `stock-brief`. The UI will show the workflow catalog, required
inputs, stage execution status, quality warnings, citations, freshness, chart
artifacts, and completed run inspection.

## Technical Context

- Language/version: Python 3.12 backend, TypeScript React/Vite frontend.
- Backend dependencies: FastAPI, Pydantic, in-memory/demo repositories, existing
  `src/api/platform` services, pytest.
- Frontend dependencies: React/Vite, existing app shell, existing workflow/result
  pages, Lightweight Charts.
- Storage: in-memory/demo repositories for Phase 02; no database migration.
...
- Performance goals: supported seeded/demo workflow runs complete in under 5
  minutes per success criteria; target interactive local/demo response under 3
  seconds for each workflow run in automated tests.
...
- Scale/scope: small seeded/demo datasets, one authenticated internal admin,
...
- Performance requirements: local/demo workflow execution target is under 3
  seconds in automated tests; production/live retrieval performance is out of
  scope and requires a later spec.
...
- `src/api/platform/memory.py`: seed VN and US stock market records, fundamental
  records or source documents, and remove gold from active supported records.
...
- `src/api/platform/workflows/collector.py`: collect required records/documents
  for one workflow run from bounded demo repositories.
...
- Use bounded seeded/demo VN and US stock repositories before native live data.
...
- Performance expectations are documented for local/demo execution.
