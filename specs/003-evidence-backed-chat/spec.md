...
Deliver a chat tab where authenticated internal users ask supported VN stock and gold finance questions. Chat must reuse the same canonical data, evidence, citations, chart artifacts, freshness metadata, and execution records established by the workflow and data-operations features.
...
- **FR-007**: System MUST provide a chat tab where generic role-based agents answer supported open-ended finance questions using the same canonical data, evidence, citations, charts, inline visualizations, and execution records as fixed workflows.
- **FR-013**: Every user-facing chat answer MUST expose citations and freshness metadata for material claims.
- **FR-014**: System MUST generate and display market chart artifacts for chat outputs that require visual price, indicator, or trend analysis.
...
- Unsupported US stock or BTC requests return a clear V1 scope limitation.
