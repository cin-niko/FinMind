...
Scope note: this Phase 001 spec originally described VN stocks and gold. ADR-0001 narrows current V1 user-facing market scope to VN stocks only (VN100 universe); gold surfaces are roadmap unless a later spec re-enables them.
...
**Independent Test**: Log in as the environment-configured internal admin, run one V1 workflow from the workflow tab using seeded/demo VN stock data, and verify that the completed result includes cited evidence, freshness metadata, at least one chart artifact when relevant, and an execution record.
...
4. Given the workflow form is a mock/demo UI, when market or instrument choices are displayed, then only V1-supported VN stock options are enabled for submission; US stock, gold, and BTC roadmap options are omitted or shown only as disabled/future preview choices before execution.
...
### User Story 4 - Inspect Real-Data Market (Priority: Parked)

An authenticated internal user opens `Market` to inspect real VN stock data through a watchlist-style dashboard with market snapshots, chart, news/source feed, freshness state, and market table.
...
2. Given the user selects a supported VN stock instrument from the system predefined watchlist, when the selection changes, then the chart, market details, freshness, and news/source feed reflect that instrument.
...
- **FR-005**: V1 MUST support VN stocks (VN100 universe) as the first implementation market scope, while preserving contracts and data modeling that can later add US stocks, gold, and BTC without replacing the core platform.
...
- **FR-008**: Phase 1 MUST maintain seeded/demo canonical storage for supported VN stock datasets with source identity, collection time, effective market time, freshness metadata, and uniqueness rules.
...
- **FR-029**: Market MUST show real seeded/demo VN stock data only, using a system predefined watchlist in V1.
...
- **FR-025**: System MUST exclude trade execution, broker connectivity, portfolio order management, self-service registration, external identity providers, US stocks, gold, and BTC from V1 user-facing market coverage.
...
- Unsupported US stock, gold, or BTC requests return a V1 scope limitation before execution where possible. Workflow forms must not let users run an unsupported market when the unsupported state is known at selection time.
...
- **SC-008**: V1 supports at least one VN stock research path across approved initial workflows.
- **SC-008a**: Workflow market controls expose only VN stock as enabled runnable choices in V1; any US stock, gold, or BTC mock/roadmap preview is disabled or explicitly out-of-scope before Run.
...
- **SC-011**: Market displays system predefined VN stock watchlist data with chart, freshness, news/source feed, and market table while containing no LLM-generated content.
...
- US stocks, gold, and BTC as supported V1 markets.
