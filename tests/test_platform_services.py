...
def test_workflow_catalog_exposes_vn_stock_and_gold_workflows(client: TestClient) -> None:
    response = client.get("/api/workflows")

    assert response.status_code == 200
    workflows = response.json()
    workflow_ids = {workflow["id"] for workflow in workflows}
    assert {"daily-market-brief", "vn-single-symbol-research", "gold-brief"} <= workflow_ids
    assert all(workflow["requires_citations"] for workflow in workflows)
    assert all("stages" in workflow for workflow in workflows)


def test_workflow_run_returns_cited_fresh_chart_result(client: TestClient) -> None:
...
def test_invalid_workflow_input_does_not_create_successful_run(client: TestClient) -> None:
    response = client.post(
        "/api/workflows/daily-market-brief/run",
